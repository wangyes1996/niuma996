...
// 优化的数据获取函数 - 减少数据量
async function getOptimizedTimeframeData(client: any, symbol: string, timeframe: string) {
  // 减少数据获取量，只获取40条数据
  ...
  // 返回15条有效数据，如果不足则返回所有有效数据
  const validDataCount = closes.length - validStartIndex;
  const returnCount = Math.min(15, validDataCount);
  ...
  return {
    ...
    metadata: {
      ...
      note: `获取40条数据用于计算，返回${returnCount}条有效数据。所有指标都有值且可靠。`
    }
  };
}
...
export async function POST(request: Request) {
  try {
    ...
    // 并行获取所有时间框架的数据
    const timeframePromises = TIME_PERIODS.map(async (timeframe) => {
      try {
        return [timeframe, await getOptimizedTimeframeData(client, upperSymbol, timeframe)];
      } catch (error) {
        console.error(`获取${timeframe}时间框架数据失败:`, error);
        return [timeframe, { error: `获取${timeframe}时间框架数据失败` }];
      }
    });

    const timeframeResults = await Promise.all(timeframePromises);
    const allData = Object.fromEntries(timeframeResults);
    ...
    const responseData = {
      ...
      metadata: {
        updateTime: new Date().toISOString(),
        note: '返回3个主要时间框架的SMA、EMA、RSI、MACD指标和交易量数据。优化了数据获取量和并行处理。'
      }
    };
